...
eqtls <- read_tsv("../data/eqtl/f2.eqtls_indep.txt", col_types = "cc-cii---------d") |>
...
eqtls_in_qtls <- eqtls |>
    ...
    select(trait, top_gwas_snp, top_eqtl_snp, gene_id, gene_name, log2_aFC) |>
...
```{r}
eqtls_in_qtls |>
    distinct(gene_id, gene_name, log2_aFC) |>
...
fc <- inner_join(
    ...
    eqtls |> select(gene_id, log2_aFC),
    by = "gene_id"
)

fc |>
    ggplot(aes(x = F0_Log2FC_bLRvsbHR, y = log2_aFC)) +
...
```{r}
fc_corr <- with(fc, cor(abs(F0_Log2FC_bLRvsbHR), abs(log2_aFC))) |> round(3)
fc |>
    ggplot(aes(x = abs(F0_Log2FC_bLRvsbHR), y = abs(log2_aFC))) +
...
For each SNP, determine whether bLR or bHR F0 rats have higher alt allele frequency. Then flip the aFC signs as needed so that it represents bLR to bHR fold change.
...
```{r}
eqtls2 <- eqtls |>
    left_join(snps, by = c("top_eqtl_snp" = "variant_id")) |>
    filter(alt_group %in% c("bLR", "bLR")) |>
    mutate(log2_aFC_bLR_bHR = if_else(alt_group == "bLR", log2_aFC, -log2_aFC))

fc2 <- inner_join(
    deg |> select(gene_id, F0_Log2FC_bLRvsbHR),
    eqtls2 |> select(gene_id, log2_aFC_bLR_bHR),
    by = "gene_id"
)
